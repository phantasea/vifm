...
const cmd_add_t cmds_list[] = {
	...
	{ .name = "open",              .abbr = "o",     .id = -1,
	  .descr = "open current file, selection or range",
	  .flags = HAS_RANGE | HAS_SELECTION_SCOPE | HAS_COMMENT,
...
help_cmd(const cmd_info_t *cmd_info)
{
	...
	const char *vi_cmd = cfg_get_vicmd(&bg, 1);  //mod by sim1
