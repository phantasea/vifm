...
read_legacy_info_file(const char info_file[])
{
	...
	JSON_Array *ratings = add_array(root, "ratings");  //add by sim1
...
load_state(JSON_Object *root, int reread)
{
	...
	load_dir_stack(root);
	load_trash(root);
	load_rating_info(root);  //add by sim1
...
store_global_options(JSON_Object *root)
{
	...
	//add by sim1 **************************************************
	append_dstr(options, format_str("%sprefervsplit", cfg.prefer_vsplit ? "" : "no"));
	...
	append_dstr(options, format_str("botmidfiller=%s", cfg.bot_mid_filler));
...
static void
load_rating_info(JSON_Object *root)
{
	JSON_Array *ratings = json_object_get_array(root, "ratings");

	int i, n;
	for(i = 0, n = json_array_get_count(ratings); i < n; ++i)
	{
		JSON_Object *obj = json_array_get_object(ratings, i);
...
str_rot_encrypt(char *str)
{
	...
	{
		str[idx] = str[idx] + CRYPT_ROT_SHIFT;
	}
...
static void
update_rating_star(rating_entry_t *entry, int star)
{
	if (NULL == entry)
	{
		return;
...
void
update_rating_info_selected(int star)
{
