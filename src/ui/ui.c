...
static const char RULER_MACROS[] = "-HraExlLPS%[]";  //mod by sim1: add char HraE
...
void print_top_mid_filler(void)
{
	...
	char topmidstr[10] = {0};
	for(int i = 0; i < getmaxx(top_line); ++i)
	{
		strcat(topmidstr, cfg.top_mid_filler);
	}

	wprint(top_line, topmidstr);
...
ui_view_title_update(view_t *view)
{
	...
	print_top_mid_filler();  //add by sim1
