...
static keys_add_info_t builtin_cmds[] = {
	...
	{WK_z WK_s,        {{&cmd_zs}, .descr = "filter selected files"}}, //add by sim1
...
static int
sel_continous(view_t *view, int *init, int *last)
{
	int bnum = 0;
	int prev = 0;
	int curr = 0;
	...
	*last = 0;
	...
	for (int i = 0; i < view->list_rows; ++i) {
		curr = view->dir_entry[i].selected;
		if (curr) {
			if (!prev) {
				bnum++;
				if (bnum > 1) {
...
cmd_O(key_info_t key_info, keys_info_t *keys_info)
{
	...
	if (tag_stat == 0 || tag_stat != g_tag_stat) {
		...
		int blocks = sel_continous(curr_view, &pos_init, &pos_last);
		if (blocks == 0) {
			...
			ui_sb_msg("More than one selection block!");
			curr_stats.save_msg = 1;
		}
	}

	//only switch boundary of the top selection block
...
{
	curr_stats.save_msg =
		fops_put(curr_view, -1, def_reg(key_info.reg), move, /*deep=*/0);
...
extern void filter_selections(view_t *view);
...
/* hide all directories = show only files */
static void
cmd_zd(key_info_t key_info, keys_info_t *keys_info)
{
	filter_directorys(curr_view);
...
/* Generate auto name filter: Filter actives. */
static void
cmd_zf(key_info_t key_info, keys_info_t *keys_info)
{
...
/* Generate auto name filter: Filter selections. */
static void
cmd_zs(key_info_t key_info, keys_info_t *keys_info)
{
	filter_selections(curr_view);
}
...
static void
cmd_zu(key_info_t key_info, keys_info_t *keys_info)
{
